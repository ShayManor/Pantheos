"""Sample fixtures for tests and the E2E reseed endpoint only.

Never seeded into a production/dev DB — ``seed()`` inserts the real scaffolding,
and ``seed_sample()`` layers this demo content on top solely for the test suite
and the ALLOW_RESEED-gated ``/api/admin/reseed`` endpoint the Playwright E2E
drives. Keeps the shipped app free of mock data while giving the gates data to
exercise ticket/monitor/Delphi flows against.
"""
